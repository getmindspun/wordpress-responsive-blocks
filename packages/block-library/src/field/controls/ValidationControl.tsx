    ...
    return (
        <>
            ...
            <TextControl
                ...
                onChange={pattern => props.setValidation({
                    pattern: pattern ? pattern : undefined,
                })}
            />
            <TextControl
                label={__('Message')}
                value={props.validation.message ? props.validation.message : 'Invalid'}
                onChange={message => props.setValidation({
...
const ValidationControl = (props: {
    ...
    return (
        <>
            <RadioControl
                label={__('Validation')}
                selected={props.validation.type ? props.validation.type : 'none'}
                options={[
                    {label: 'None', value: 'none'},
                    {label: 'Simple', value: 'simple'},
                    {label: 'Custom', value: 'custom'},
                ]}
                onChange={type => props.setValidation({
                    type: type !== 'none' ? type as Validation['type'] : undefined,
                })}
            />
            {(props.validation.type === 'simple') ? <SimpleValidationControl {...props} /> : null}
            {(props.validation.type === 'custom') ? <CustomValidationControl {...props} /> : null}
        </>
    );
}
