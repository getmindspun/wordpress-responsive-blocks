	...
	return (
		<div className={'wpx--colspan-control'}>
			...
			<DeviceTypeContainer deviceType={'Desktop'}>
				...
				/>
				{isFixed(props.attributes.desktop) && (
					<RangeControl
						value={colToRange(props.attributes.desktop)}
						onChange={(value) => {
							props.setAttributes({
								desktop: value ? value.toString() : undefined,
							});
						}}
						min={1}
						max={12}
			...
			<DeviceTypeContainer deviceType={'Tablet'}>
				...
				/>
				{isFixed(props.attributes.tablet) && (
					<RangeControl
						value={colToRange(props.attributes.tablet)}
						onChange={(value) => {
							props.setAttributes({
								tablet: value ? value.toString() : undefined,
							});
						}}
						min={1}
						max={12}
			...
			<DeviceTypeContainer deviceType={'Mobile'}>
				...
				/>
				{isFixed(props.attributes.mobile) && (
					<RangeControl
						value={colToRange(props.attributes.mobile)}
						onChange={(value) => {
							props.setAttributes({
								mobile: value ? value.toString() : undefined,
							});
						}}
						min={1}
						max={12}
