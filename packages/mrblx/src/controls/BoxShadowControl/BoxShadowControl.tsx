...
export type BoxShadowProperties = Pick<
	BlockCSSProperties,
	'boxShadow' | 'tabletBoxShadow' | 'mobileBoxShadow'
>;
...
export type BoxShadowControlProps = {
	...
	setAttributes: (
		attributes: Partial<BoxShadowControlProps['attributes']>
	) => void;
...
const BoxShadowControl = (props: BoxShadowControlProps) => {
	...
	return (
		<div className="mrblx--box-shadow-control">
			<ControlHeader
				title={props.label}
			...
			{(deviceType === 'Desktop' ||
				...
				<ToggleControl
					...
					onChange={(isChecked) => {
						props.setAttributes({
							[prop(props.isResponsive, deviceType)]: isChecked
								? DEFAULT
								: undefined,
						});
					}}
				...
				<>
					{deviceType !== 'Desktop' && (
						<ToggleControl
							...
							checked={
								props.attributes[
									prop(props.isResponsive, deviceType)
								] !== 'none'
							}
							onChange={(isChecked) => {
								props.setAttributes({
									[prop(props.isResponsive, deviceType)]:
										isChecked ? undefined : 'none',
								});
							}}
						/>
					)}
					{props.attributes[prop(props.isResponsive, deviceType)] !==
						'none' && (
						<>
							<ToggleControl
