...
export function toBorderShorthand(border?: Border): string | undefined {
	...
	const result = border.width ? [border.width] : [];
...
export function fromBorderShorthand(
...
export function showClear(attributes: BlockCSSProperties, deviceType: string) {
	switch (deviceType) {
		case 'Tablet':
			return (
				attributes.tabletBorderTop !== undefined ||
		...
		case 'Mobile':
			return (
				attributes.mobileBorderTop !== undefined ||
	...
	return (
		attributes.borderTop !== undefined ||
