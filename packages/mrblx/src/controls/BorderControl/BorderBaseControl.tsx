...
const BorderBaseControl = (props: BorderBaseControlProps) => {
	...
	const onChange = (newBorders: AnyBorder) => {
		if (!newBorders) {
			...
			(newBorders as Borders).top ||
			(newBorders as Borders).right ||
			(newBorders as Borders).bottom ||
			(newBorders as Borders).left
		) {
	...
	return <BorderBoxControl onChange={onChange} value={value} />;
};
