...
export type FontAppearanceControlProps = {
	...
	setAttributes: (
		attributes: Partial<FontAppearanceControlProps['attributes']>
	) => void;
};
...
const FontAppearanceControl = (props: FontAppearanceControlProps) => {
	...
	return (
		<div className="mrblx--font-appearance-control">
			<ControlHeader
				title="Appearance"
				onClear={
					props.attributes.fontWeight !== undefined ||
					props.attributes.fontStyle !== undefined
						? onClear
						: undefined
				}
			/>
			<CustomSelectControl
				label={undefined}
				options={asOptions()}
				onChange={({ selectedItem }: { selectedItem: Option }) =>
					onChange(selectedItem)
				}
				value={find(
					props.attributes.fontWeight as number | undefined,
					props.attributes.fontStyle as
						| 'normal'
						| 'italic'
						| undefined
				)}
