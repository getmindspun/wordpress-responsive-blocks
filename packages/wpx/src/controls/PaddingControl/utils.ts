...
function calculateIsMixed(attributes: BlockCSSProperties, isResponsive: boolean, deviceType: string) {
    if (isResponsive) {
        switch (deviceType) {
            case 'Tablet':
                return (attributes.tabletPaddingTop !== attributes.tabletPaddingBottom) ||
                    (attributes.tabletPaddingTop !== attributes.tabletPaddingLeft) ||
                    (attributes.tabletPaddingTop !== attributes.tabletPaddingRight);
            case 'Mobile':
                return (attributes.mobilePaddingTop !== attributes.mobilePaddingBottom) ||
...
export function areAllPaddingsEqual(attributes: CSSProperties) {
    return (attributes.paddingTop === attributes.paddingBottom) &&
